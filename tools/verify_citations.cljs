...
;;
;; Re-fetches the official eCFR APIs and checks six things:
;;
...
;;      `:statute/verified-label`. An empty path denotes the title root itself.
;;
...
;;      whose operative claims have been gutted.
;;
;;   3. COUNTS -- for every `occurrence-counts` entry, the pattern occurs
;;      EXACTLY the recorded number of times in the fetched document.
;;
;;      This is the check the sibling USA-TREASURY leaf recorded as missing
;;      from its own gate, and this catalog cannot be verified without it: its
;;      central finding is that the operative federal clauses contain the
;;      string `Security and Exchange Commission` twice and `Securities and
;;      Exchange Commission` zero times. Presence/absence cannot express that,
;;      and worse, a presence check would keep passing after the government
;;      corrected the spelling -- which is precisely the day this catalog's
;;      advice would need to change.
;;
;;      An expectation of 0 must carry `:count/control`, for the same reason an
;;      absence must: an empty document satisfies every claim of the form `this
;;      string occurs zero times`.
;;
;;   4. NEGATIVE (structure) -- for every absence:
...
;;
;;   5. NEGATIVE (text) -- for every `:absence/absent-text`, the pattern does
...
;;      matched` looks exactly like `confirmed still absent`.
;;
;;   6. FLOOR -- a run that checked fewer than `--min` headings, `--min-quotes`
;;      quotes, `--min-counts` counts, `--min-absences` absences, or zero of
;;      any kind, is a could-not-answer.
;;
;; Why walk the tree rather than match the URL. Hierarchical CFR identifiers
;; nest as substrings of one another (part `20` is a prefix of part `200`, part
;; `24` of part `240`), so a string match can succeed against the wrong node.
;; Walking explicit [type identifier] steps cannot pass by accident. A step may
;; be `"*"`, used only where eCFR generates the identifier rather than the CFR
;; citing it (the `subject_group` nodes inside 17 CFR part 240 have identifiers
;; like `ECFR821bdfb7bc8017f`); a wildcard that resolves to more than one node
;; is reported as a failure, not silently taken.
;;
...
;;   * exit 1 -- answered, and at least one citation, quote, count or absence
;;               is wrong.
...
;;       [--min-counts N] [--min-absences N] [--gap-ms N] [--quiet]
...
(def min-citations (js/parseInt (flag-val "--min" "20") 10))
(def min-quotes    (js/parseInt (flag-val "--min-quotes" "8") 10))
(def min-counts    (js/parseInt (flag-val "--min-counts" "7") 10))
...
(def counts-spec   (read-form-after src "(def occurrence-counts"))
...
(defn section-text
  ...
  This is the ONE instrument. Quotes and counts are both measured with it, so a
  number recorded in the catalog and a number produced here are commensurable."
  [xml]
  (-> xml
      (str/replace #"<[^>]*>" "")
...
(defn n-occurrences
  "Non-overlapping occurrences of `pat` in `s`. Literal by default; `regex?`
  switches to pattern matching for counts whose subject contains CFR
  parentheses such as `section 13(a) or 15(d)`."
  [s pat regex?]
  (if regex?
    (count (re-seq (re-pattern pat) s))
    (loop [i 0 c 0]
      (let [j (.indexOf s pat i)]
        (if (neg? j) c (recur (+ j (count pat)) (inc c)))))))
...
(defn walk-node
  ...
  exactly one node. An empty path denotes the title root. Returns
  {:node n} | {:missing path} | {:ambiguous n}."
  [tree node-path]
...
(defn doc-url
  "Full-text URL for {title, part, optional section}, or nil if the title has
  no declared endpoint."
  [cfr-title part section]
  (when-let [base (get full-text-api cfr-title)]
    (str base "?part=" part (when section (str "&section=" section)))))
...
(defn quotes
  ...
  than one load-bearing claim, so each span becomes its own check and losing one
  is a failure rather than a silently smaller gate."
  []
  (for [[iso es] catalog, e es
    ...
    {:what (str iso " " (:statute/id e) " quote#" (inc i))
     ...
     :url (doc-url (:statute/cfr-title e) (:statute/quote-part e) (:statute/quote-section e))
     :quote q}))

(defn occurrence-checks
  "Every exact-count claim this run must confirm."
  []
  (for [c counts-spec]
    {:what (str "count " (:count/id c))
     :cfr-title (:count/cfr-title c)
     :url (doc-url (:count/cfr-title c) (:count/part c) (:count/section c))
     :pattern (:count/pattern c)
     :regex? (boolean (:count/regex? c))
     :expect (:count/expect c)
     :control (:count/control c)}))
...
(defn text-absences
  ...
  (for [a absences :let [t (:absence/absent-text a)] :when t]
    {:what (str "absence " (:absence/id a))
     ...
     :url (doc-url (:statute/cfr-title t) (:statute/part t) (:statute/section t))
...
(defn -main []
  (let [pos   (vec (positives))
        ...
        _ (when (empty? cnts) (die 2 "CANNOT-ANSWER: catalog produced zero counts to check"))
        ...
        _ (doseq [c cnts]
            (when-not (:url c)
              (die 2 "CANNOT-ANSWER: no full-text endpoint declared for CFR title"
                   (:cfr-title c) "needed by" (:what c)))
            (when-not (number? (:expect c))
              (die 2 "CANNOT-ANSWER:" (:what c) "has no numeric :count/expect"))
            (when (and (zero? (:expect c)) (not (:control c)))
              (die 2 "CANNOT-ANSWER:" (:what c)
                   "expects zero occurrences but declares no :count/control --"
                   "an empty document would confirm it for free")))
        _ (doseq [t tabs]
            (when-not (:url t)
              (die 2 "CANNOT-ANSWER: no full-text endpoint declared for CFR title"
        ...
        doc-urls     (distinct (concat (map :url qs) (map :url cnts) (map :url tabs)))
        ...
        (.then
         (fn [bodies]
           (let [trees (into {} (for [t need-struct] [t (js->clj (get bodies t))]))
             ...
             ;; 3. counts
             (doseq [{:keys [what url pattern regex? expect control]} cnts]
               (let [t (get docs url)]
                 (if (str/blank? t)
                   (cannot! "CANNOT-CHECK" what "-- empty document from" url)
                   (let [ctrl-ok (or (nil? control) (pos? (n-occurrences t control false)))]
                     (if-not ctrl-ok
                       (cannot! "CONTROL-FAILED" what "-- control" (pr-str control)
                                "does not occur in the fetched text, so the document is not the"
                                "one this count is about and a count of zero proves nothing")
                       (let [actual (n-occurrences t pattern regex?)]
                         (when-not (= actual expect)
                           (fail! "COUNT-DRIFT" what "-- expected" expect
                                  "occurrence(s) of" (pr-str pattern) "but found" actual
                                  "in" url))))))))
             (say "  counts checked:" (count cnts))

             ;; 4. structure negatives
             ...
             (when (< (count qs) min-quotes)
               (die 2 "CANNOT-ANSWER: only" (count qs) "quotes checked, below --min-quotes"
                    min-quotes))
             (when (< (count cnts) min-counts)
               (die 2 "CANNOT-ANSWER: only" (count cnts) "counts checked, below --min-counts"
                    min-counts))
             ...
             (if (seq @fails)
               ...
               (do (say "\nVERIFIED:" (count pos) "headings," (count qs) "quotes,"
                        (count cnts) "counts," (+ (count negs) (count tabs))
                        "absences -- all live against eCFR.")
                   (js/process.exit 0)))))))))
